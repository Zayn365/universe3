...
const PageConnectWallet = ({ }) => {
  ...
  const { sdk, connected, account, extensionActive } = useSDK();

  const getWalletFunction = async () => {
    if (!extensionActive) {
      return toast.error("Please install metamask extension");
    }
    let wallet = account;
    if (!connected || !wallet) {
      const res: any = await sdk?.connect();
      wallet = res?.[0] as string;
    }
    if (wallet) {
