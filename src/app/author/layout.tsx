...
//   useAuth();
//   const { user, types, setType } = useUserContext();
//   console.log(types);
//   const pathname = usePathname();
//   let navs: { name: string; href: Route; onClick: () => void }[] = [
//     {
//       name: "NFTS Owned",
//       href: "/author?All",
//       onClick: () => {
...
//               <NcImage
//                 src={user && user?.img ? user?.img : nftsImgs[2]}
  ...
  return (
    <div className={`nc-AuthorPage `}>
      ...
      <div className="w-full">
        ...
        <div className="container -mt-10 lg:-mt-16">
          <div className="relative bg-white dark:bg-neutral-900 dark:border  justify-center items-center  dark:border-neutral-700 p-5 lg:p-8 rounded-3xl md:rounded-[40px] shadow-xl flex flex-col md:flex-row">
            <div className="w-32 lg:w-42 flex-shrink-0 mt-12 sm:mt-0">
              <NcImage
                src={nftsImgs[2]}
                containerClassName="aspect-w-1 aspect-h-1 rounded-full overflow-hidden z-0 relative"
                // fill
                sizes="200px"
              />
            </div>
            <div className="pt-5 md:pt-1 md:ml-6 xl:ml-14 flex-grow">
              <div className="max-w-screen-sm ">
                <h2 className="inline-flex items-center text-2xl sm:text-3xl lg:text-4xl font-semibold">
                  <span>
                    {user && user.wallet?.slice(0, 4)}...
                    {user && user.wallet?.slice(4, 8)}
                  </span>
                </h2>
                <div className="flex items-center text-sm font-medium space-x-2.5 mt-2.5 text-green-600 cursor-pointer">
                  <span className="text-neutral-700 dark:text-neutral-300">
            ...
            <div className="absolute md:static left-5 top-4 sm:left-auto sm:top-5 sm:right-5 flex flex-row-reverse justify-end"></div>
          </div>
        </div>
      </div>
      {/* ====================== END HEADER ====================== */}
      ...
      <div className="container py-16 lg:pb-28 lg:pt-20 space-y-16 lg:space-y-28">
        <main>
          <div className="flex flex-col lg:flex-row justify-between ">
            <div className="flex space-x-0 sm:space-x-1.5 overflow-x-auto ">
              {navs.map((item, index) => {
                const active = item.href === `${pathname}?type=${queryType}`;
                return (
                  <Link
                    key={index}
                    onClick={item.onClick}
